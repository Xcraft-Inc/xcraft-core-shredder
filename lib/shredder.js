...
class Shredder {
  ...
  static mutableReducer(mutator) {
    return (state, action) =>
      state.withMutations(mutableState => mutator(mutableState, action));
  }
  ...
  applyChange(path, change) {
    return this.set(path, applyChanges(this.get(path, []).toArray(), change));
  }
  ...
  applyForm(value) {
    ...
    Object.keys(value)
      ...
      .forEach(name => {
        const fields = new Shredder(value[name]);
        fields
          .filter(item => item.key !== '$form')
          .forEach(item => {
  ...
  /* Should be removed but it generates a lot of regressions */
  map(...args) {
  ...
  _pathShredder(path) {
    if (!Array.isArray(path)) {
      path = toPath(path).map(p => (!isNaN(p) ? parseInt(p) : p));
    }
    return path;
  }
