...
class Shredder {
  ...
  get linq () {
    return Enumerable.from (this._select ((i, v) => v));
  }

  where (...args) {
    return Enumerable.from (this._select ((i, v) => v)).select (...args);
  }
  ...
  merge (path, value) {
    ...
    const nextState = this._setListFromPath (path, this.state).mergeDeepIn (
      path,
      fromJS (value)
    );
